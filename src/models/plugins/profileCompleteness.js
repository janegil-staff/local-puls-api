...
var REQUIREMENTS = [
    { field: 'username', isPresent: (doc) => !!String(doc.username || '').trim() },
    { field: 'dateOfBirth', isPresent: (doc) => !!doc.dateOfBirth },
    { field: 'gender', isPresent: (doc) => !!doc.gender },
    {
        field: 'location',
        isPresent: (doc) => Array.isArray(doc.location?.coordinates)
...
function updateTouchesWatched(update) {
    ...
    for (var operator of Object.keys(update)) {
        if (operator.startsWith('$')) keys.push(...Object.keys(update[operator] || {}));
        else keys.push(operator);
    }

    return keys.some((key) =>
        WATCHED_PATHS.some((watched) => key === watched || key.startsWith(watched + '.'))
    );
}
...
export function profileCompleteness(schema) {
    ...
    async function recomputeAfterUpdate() {
        ...
        var docs = await this.model.find(filter).select(
            [...WATCHED_PATHS, 'profileComplete'].join(' ')
        );
