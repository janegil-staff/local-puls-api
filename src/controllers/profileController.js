...
function coercePhotos(photos) {
  if (!Array.isArray(photos))
    throw ApiError.badRequest("photos must be an array");
...
export const updateProfile = asyncHandler(async (req, res) => {
  ...
  if (dob !== undefined) {
    ...
    if (age < MIN_AGE)
      throw ApiError.badRequest("You must be at least 18 to use this app");
    user.dob = dob;
  }

  if (req.body.show !== undefined) {
    user.preferences.show = req.body.show;
    user.preferences.showSetByUser = true;
  }

  if (req.body.gender !== undefined) {
    user.gender = req.body.gender;

    if (!user.preferences?.showSetByUser) {
      user.preferences.show = defaultShowFor(user.gender);
    }
  }
  ...
  if (showOnlineStatus !== undefined) {
    if (typeof showOnlineStatus !== "boolean")
      throw ApiError.badRequest("showOnlineStatus must be a boolean");
    user.showOnlineStatus = showOnlineStatus;
  }
  if (showDistance !== undefined) {
    if (typeof showDistance !== "boolean")
      throw ApiError.badRequest("showDistance must be a boolean");
...
export const updatePreferences = asyncHandler(async (req, res) => {
  ...
  if (show !== undefined) {
    if (!ORIENT_SHOW.includes(show))
      throw ApiError.badRequest("Invalid show preference");
    user.preferences.show = show;
  }
  if (ageMin !== undefined)
    user.preferences.ageMin = Math.max(MIN_AGE, Number(ageMin));
  if (ageMax !== undefined)
    user.preferences.ageMax = Math.min(MAX_AGE, Number(ageMax));
  ...
  if (maxDistanceKm !== undefined) {
    if (maxDistanceKm === null) {
      ...
      if (!Number.isFinite(n))
        throw ApiError.badRequest("maxDistanceKm must be a number");
      user.preferences.maxDistanceKm = Math.min(
        MAX_DISTANCE_KM,
        Math.max(MIN_DISTANCE_KM, n),
      );
    }
  }
...
export const updateLocation = asyncHandler(async (req, res) => {
  ...
  if (lng == null || lat == null)
    throw ApiError.badRequest("lng and lat required");
...
export const deleteAccount = asyncHandler(async (req, res) => {
  ...
  const [user, posts, messages] = await Promise.all([
    ...
    Message.find({ sender: uid, imageUrl: { $exists: true } }).select(
      "imageUrl",
    ),
  ]);
