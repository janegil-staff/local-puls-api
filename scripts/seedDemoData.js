...
// for any future "shared interests" comparison — a straight set intersection
// only works if both sides are normalised the same way.
function normaliseInterests(list, username) {
  ...
  if (unknown.length) {
    ...
    // refuse. With an enum on the model this would block every subsequent
    // save on that document, not just this field.
    throw new Error(
...
async function loadManifest() {
  const raw = await fs.readFile(MANIFEST_PATH, "utf8").catch(() => {
    throw new Error(
      `Manifest not found at ${MANIFEST_PATH}. Run uploadSeedAssets.js first.`,
    );
  });
  const m = JSON.parse(raw);
  if (!m?.posts || !m?.avatars)
    throw new Error('Manifest must contain "avatars" and "posts".');
  return m;
}
...
async function seedPosts(users, manifest) {
  ...
  console.log(
    `With images: ${withImg} | text-only: ${inserted.length - withImg}`,
  );
  return inserted;
}

// How often each interest appears across the seed set. Printed at the end so
// a thin spread is visible before you demo the app rather than during it.
...
async function run() {
  ...
  console.log("");
  console.log("Interest spread (shared interests need overlap to be visible):");
  for (const [id, n] of interestSpread()) {
    console.log(`  ${id.padEnd(14)} ${"█".repeat(n)} ${n}`);
  }
}

run()
  .catch((e) => {
    console.error("Demo data seed failed:", e.message);
    process.exitCode = 1;
  })
  .finally(async () => {
