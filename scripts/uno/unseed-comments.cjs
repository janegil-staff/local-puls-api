...
//
// Removes ALL seeded comments (isSeedPost: true) created by seed-comments.cjs.
...
async function main() {
  ...
  const toRemove = await comments.find({ isSeedPost: true }).project({ _id: 1, post: 1 }).toArray();
  ...
  const result = await comments.deleteMany({ isSeedPost: true });
