...
//
// Cleanup:  node -e "require('mongoose').connect(process.env.MONGO_URI).then(async m=>{await m.connection.collection('comments').deleteMany({isSeed:true});process.exit(0)})"
//   or:     Comment.deleteMany({ isSeed: true })
//
...
async function main() {
  ...
  // Option A — a post has its own isSeed flag:      { isSeed: true }
  ...
  for (const post of seededPosts) {
    ...
    const existing = await comments.countDocuments({ post: post._id, isSeed: true });
